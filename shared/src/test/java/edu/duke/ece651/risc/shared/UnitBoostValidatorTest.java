...
public class UnitBoostValidatorTest {
  ...
  public void test_UBVunits() {
    ...
    // ubv.validateOrders(orders); // overall fail

    // successive orders suceeding
    Board boardCopy = (Board) DeepCopy.deepCopy(board);
    ...
    orders.add(boost2);
    Unit unitBoost3 = new Unit(listOfUnitInts(0, 0, 0, 4, 0, 0, 0)); // valid: units will be [0, 0, 0, 0, 5, 1, 1]
  ...
  public void test_UBVcorners() {
    List<Region> regions = getRegions(1, 1, 1, 1, 1, 1); // 1 unit of each type in each region
    Board board = new Board(regions);
    regions.get(0).getRegionByName(board, "Earth").getOwner().getResources().getTechResource().addTech(300); // 15 + 125 = 140
    Board boardCopy = (Board) DeepCopy.deepCopy(board);
